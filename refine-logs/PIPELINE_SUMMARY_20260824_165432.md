...
**Final Verdict**：`REVISE`；fresh same-family M2 review `RELEASE_MOCK_ONLY`；scientific execution `BLOCKED`；paper `NOT READY`。  
...
- `FINAL_PROPOSAL.md`、`MANIFEST.md` 与代码均未由本规划步骤修改。
...
| M2 | blind replay | COMPLETE_VERIFIED (mock only) | fresh review + 130 synthetic tests；real evaluator isolation pending |
...
1. 保持真实 run 为零；M2 synthetic blind replay、maturity、suffix invariance、crash no-resend与fresh review均已完成。
2. 非联网实现 production Ark adapter contract与process-isolated event/label service，并先通过canary/failure injection。
3. 等待 P1-Ren/P1-Patrizi独立批准；收到批准前不下载。
4. 等待轮换凭据与 P3 Gate-2批准；收到批准前不做authenticated discovery或probe。
...
在保持零真实调用的前提下实现 production adapter与进程隔离 evaluator；同时停在 **P1/P3 human checkpoint**。只有 Data Gate、真实 whole-unit split重算、P2 numerical/power seal、P3 authenticated capability 与 P4 spend批准全部通过后，才进入 `/run-experiment` 的真实 accuracy/API阶段。
