...
**Updated**：2026-08-20 17:14:23 +0800  
...
| R021 | M2 | 正式设计仿真 | ≥2,000 repeats/grid | physical-unit simulation | power/coverage/champion diagnostics | MUST | BLOCKED | Data Gate 后冻结仿真参数并执行；通过后才能完成 Freeze B；当前不执行 |
...
| R023 | M2 | 结果到主张裁决 | fresh result-to-claim reviewer | final audit + evidence ledger | C1–C5 yes/partial/no | MUST | COMPLETE_CLAIM_CEILING | C1=yes（sanity only）；C2/C3/C4=no；C5=partial：下一步仅 parser/Data Gate，正式 Design Gate 后才能 Freeze B；见 `CLAIMS_FROM_RESULTS.md` |
...
- 独立 result-to-claim 裁决仅支持实现 sanity；下一阶段若获批是 reference-aware parser/Data Gate，不是直接进入 Freeze B。
