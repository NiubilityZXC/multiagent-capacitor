...
本轮执行以下 selected suite，结果 `40 passed in 0.56s`：
...
这只支持M0/M1及M2部分合同；没有数据/API/accuracy结果。
...
| CAP-M0-002 | M0 | authority/fallback isolation | H1/RF1/RC1/ACT1/IF1 | synthetic | b_star=N0；RC1=8；ACT1=19；76,969/quotient19 | MUST | COMPLETE_VERIFIED | — |
...
| CAP-M2-001 | M2 | durable attempt lifecycle | accuracy_v1 ledger | synthetic | STARTED before provider；no retry；late/crash fallback | MUST | PARTIAL_VERIFIED | integrate event service |
| CAP-M2-002 | M2 | blind replay E2E | reveal→commit→maturity→score | synthetic hidden suffix | no reveal before atomic marker；common keys | MUST | PLANNED | KILL_PROTOCOL on fail |
| CAP-M2-003 | M2 | maturity denominator | PLANNED/MATURED/NEVER_MATURED | synthetic stream | one execution + one maturity status/key | MUST | PLANNED | CAP-M2-002 |
| CAP-M2-004 | M2 | suffix/identity invariance | tainted/private fixtures | synthetic | byte-identical packet/prediction | MUST | PLANNED | KILL_PROTOCOL on fail |
...
| CAP-P3-002 | P3 | capability probes | each authenticated candidate | synthetic packet | 3–5 repeats、strict schema/model/usage/latency | MUST | BLOCKED_P3-001 | failed model excluded |
...
1. 完成 `CAP-M2-002/003/004`，把现有 ledger unit tests提升为端到端 blind replay。
2. 生成 P1 human decision packet；在批准前不下载任何 payload。
3. P1通过后先完成 Ren row-level Gate；Patrizi独立裁决。
4. P2冻结 numerical/metric/power；只有通过后才进入 P3 authenticated Ark Gate。
