...
**状态**：`M3_MOCK_RELEASE / SCIENCE_BLOCKED / PAPER_REVISE`
...
| `refine-logs/EXPERIMENT_CODE_REVIEW_20260824_182331.md` | `d2db9ed2a9df6d2ad82bb6b3f19e9141a246c5d4b02c731259b5e7ddc845154c` | M3 Ark/evaluator adversarial review与post-fix分层裁决 |
...
本轮本地复核结果为 `204 passed`。M0–M2 causal/action/replay 合同、M3 离线 Ark binding、runner-owned usage、ledger short-write、进程分离 evaluator、early-FINALIZE零标签写入、journal/config/tamper/crash fixtures 均通过。它仍是 synthetic/mock harness evidence，不是 qualified-data accuracy、远端 API 或 RUL 证据。
...
| 当前放行 | M0/M1/M2 complete-verified；M3 evaluator仅local-process、Ark仅mock；P1/P3需要人工Gate；P4/P5禁止运行 |
...
|---|---|---|---|---|---|
| **M0 — mock contracts** | schema/action/fault correctness | strict packets/arms、19/96隔离、SHIFT/INFLATE、fallback states、no-network/fault tests | 相关tests全部PASS且无secret/network side effect | `COMPLETE_VERIFIED` | CPU-only，0 API |
...
| **M2 — blind replay** | reveal→commit→maturity闭环 | planned/maturity manifest、STARTED fsync、crash/late recovery、prediction-before-reveal、scorer seal | end-to-end hidden-suffix test与tamper test全部PASS | `COMPLETE_VERIFIED`（synthetic/mock） | CPU-only，0 API |
| **M3 — provider/evaluator boundary** | direct/hybrid LLM调用边界与标签隔离 | typed Ark envelope、usage/deadline/model binding、process evaluator、IPC/replay/finalize attacks | fresh review + post-fix re-review；全量204 tests | `RELEASE_MOCK_ONLY` integrated；evaluator=`RELEASE_LOCAL_PROCESS_BOUNDARY` | CPU-only，0 API |
...
- **CPU**：M0–M3、P2 core numerical、P6；优先cached deterministic preprocessing。
...
- [x] M2 end-to-end blind replay PASS（synthetic/mock）
- [x] M3 offline Ark/evaluator adversarial review与post-fix re-review
