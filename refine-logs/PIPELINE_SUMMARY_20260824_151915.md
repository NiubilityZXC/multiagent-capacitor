...
**Final Verdict**：`REVISE`；mock implementation `GO`；scientific execution `BLOCKED`；paper `NOT READY`。  
...
| M2 | blind replay | PARTIAL_VERIFIED | reveal→commit→maturity E2E |
...
1. `CAP-M2-002`：synthetic hidden-suffix end-to-end blind replay。
2. `CAP-M2-003`：PLANNED→execution→maturity denominator state-machine test。
3. `CAP-M2-004`：suffix/identity mutation invariance + tamper kill test。

完成后只生成 P1 human decision packet；不能自动下载。
...
完成 M2 端到端 blind replay，并准备 **P1 human acquisition decision packet**。在 Data Gate、P2 numerical/power seal 与 P3 authenticated capability Gate全部通过前，不进入 `/run-experiment` 的真实 accuracy/API阶段。
